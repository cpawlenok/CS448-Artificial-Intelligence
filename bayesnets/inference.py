...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        bayesTables = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        for eliminateVar in eliminationOrder:                       # iterate through the variables being eliminated
            #print(joinFactorsByVariable(bayesTables, eliminate))
            bayesTables, newTables = joinFactorsByVariable(bayesTables, eliminateVar)   # took me forever to realize this returns 2 things
            #print(newTables)
            if len(newTables.unconditionedVariables())>1:           # at first I was using zero but we only need to
                temp = eliminate(newTables, eliminateVar)           # eliminate again if there's more than one
                                                                    # unconditioned variable left
                #print(newTables)
                bayesTables.append(temp)
            #print(newTables)

        return normalize(joinFactors(bayesTables))                  # first join and then normalize new factors
